...
class STSEval(object):
    ...
    def run(self, params, batcher):
        ...
        for dataset in self.datasets:
            sys_scores = []
            input1, head1, rel1, input2, head2, rel2, gs_scores = self.data[dataset]

            for ii in range(0, len(gs_scores), params.batch_size):
                batch1 = input1[ii:ii + params.batch_size]
                batch2 = input2[ii:ii + params.batch_size]
                batch_head1 = head1[ii:ii + params.batch_size]
                bacth_head2 = head2[ii:ii + params.batch_size]
                batch_rel1 = rel1[ii:ii + params.batch_size]
                batch_rel2 = rel2[ii:ii + params.batch_size]

                # we assume get_batch already throws out the faulty ones
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    enc1 = batcher(params, batch1, batch_head1, batch_rel1)
                    enc2 = batcher(params, batch2, bacth_head2, batch_rel2)
...
class STSBenchmarkEval(STSEval):
    def __init__(self, task_path, seed=1111):
        ...
        test = self.loadFile(os.path.join(task_path, 'sts-test.json'))
    ...
    def loadFile(self, fpath):
        sick_data = {'X_A': [], 'head_A':[], 'rel_A':[], 'X_B': [], 'head_B':[], 'rel_B':[], 'y': []}
        with io.open(fpath, 'r', encoding='utf-8') as f:
            data = json.load(f)
            for line in data:
                # text = line.strip().split('\t')
                sick_data['X_A'].append(line['sent1'].split())
                sick_data['head_A'].append(line['head1'])
                sick_data['rel_A'].append(line['rel1'])
                sick_data['X_B'].append(line['sent2'].split())
                sick_data['head_B'].append(line['head2'])
                sick_data['rel_B'].append(line['rel2'])
                sick_data['y'].append(line['score'])

        sick_data['y'] = [float(s) for s in sick_data['y']]
        self.samples += sick_data['X_A'] + sick_data['head_A'] + sick_data['rel_A'] + sick_data["X_B"] + sick_data['head_B'] + sick_data['rel_B']
        return (sick_data['X_A'], sick_data['head_A'], sick_data['rel_A'], sick_data["X_B"], sick_data['head_B'], sick_data['rel_B'], sick_data['y'])
