...
from model.graph_encoder import GNNRelationModel
...
class MLPLayer(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size).to(device="cuda:0")
        self.activation = nn.Tanh()

    def forward(self, features, **kwargs):
        x = self.dense(features.to(device="cuda:0"))
        x = self.activation(x)
...
def cl_forward(cls,
    ...
    dep_rel_matrix = torch.from_numpy(np.concatenate(dep_rel_matrix,
                                                           axis=0))
    # print("Shape of dep_rel_matrix: ", shape(dep_rel_matrix))
    # device = torch.device("cuda:0")
    # adj_matrix = Variable(adj_matrix.to(device))
    # dep_rel_matrix = Variable(dep_rel_matrix.to(device))
