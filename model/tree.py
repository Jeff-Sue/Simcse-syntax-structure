...
def head_to_tree(tokenizer, token, head, len_, prune, subj_pos, obj_pos, dep_rel, wp_conversion=True):
    """
    Convert a sequence of head indexes into a tree object.
    """
    assert len(head) == len_
    ...
    if prune < 0:
        for i in range(len(token)):
            i_ids = tokenizer(token[i])['input_ids']
            for t in range(len(i_ids)-2):
                head1.append(int(head[i]))
                dep_rel1.append(dep_rel[i])
        head1.append(0)
        dep_rel1.append(0)
        if len(head1) > 128:
            head1 = head1[:128]
